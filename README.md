...
```js
// Original code from https://github.com/Bappyllcg/html-form-to-google-sheet-with-array-data
// Updated for 2023 and ES6 standards
  ...
  try {
    ...
    const newRow = headers.map(function(header) {
      if (header === 'Date') {
        ...
        const paramName = header.slice(0, -2); // Remove '[]'
        const values = e.parameters[paramName + '[]'] || [];
        return values.join('\n'); // Use new line as separator
        //return values.join(', '); // Use comma as separator
      } else {
    ...
    return ContentService
      .createTextOutput(JSON.stringify({ 'result': 'Data Inserted!', 'row': nextRow }))
